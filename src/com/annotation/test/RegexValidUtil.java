    ...
    public static boolean check(Object obj) throws Exception {
        ...
        for (Field field : fields){
            ...
            if (field.isAnnotationPresent(RegexValid.class)){
                ...
                // 如果 value 为空字符串 说明灭有注入自定义正则表达式 改用 policy 属性
                ...
                if (fieldObj == null){
                    sb.append(String.format("%s 类中的 %s 字段不能为空！", obj.getClass().getName(), field.getName()));
                    result = false;
                } else {
                    if (fieldObj instanceof String){
                        ...
                        sb.append("\n").append(String.format("%s 类中的 %s 字段不是字符串类型，不能使用此注解校验！",
